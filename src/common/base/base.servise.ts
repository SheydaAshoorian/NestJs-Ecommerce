...
@Injectable()
export abstract class BaseService<T, CreateDto, UpdateDto, Entity> {
  ...
  constructor(
    ...
    private readonly entityClass: new (data: T) => Entity,
  ) {
  ...
  async create(createDto: CreateDto): Promise<Entity> {
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2002') {
          const field = (error.meta?.target as string[])?.join(', ') || 'فیلد';
          throw new BadRequestException(
            `رکورد تکراری: مقدار وارد شده برای ${field} قبلاً ثبت شده است`,
          );
        }
      }
  ...
  async remove(id: number): Promise<T> {
    try {
      ...
      if (error.code === 'P2025') {
        throw new BadRequestException(
          `رکورد با شناسه ${id} یافت نشد که حذف شود.`,
        );
      }
